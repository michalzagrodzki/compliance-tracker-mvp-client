...
export const useAuditSession = () => {
  ...
  const fetchAllSessions = useCallback(
    async (skip: number = 0, limit: number = 10) => {
      ...
      try {
        const fetchedSessions = await auditSessionService.getAllSessions(
          skip,
          limit
        );
  ...
  const fetchUserSessions = useCallback(
    async (userId: string, skip: number = 0, limit: number = 10) => {
      ...
      try {
        const fetchedSessions = await auditSessionService.getUserSessions(
          userId,
          skip,
          limit
        );
  ...
  const fetchSessionsByStatus = useCallback(
    async (isActive: boolean, skip: number = 0, limit: number = 10) => {
      ...
      try {
        const fetchedSessions = await auditSessionService.getSessionsByStatus(
          isActive,
          skip,
          limit
        );
  ...
  const fetchSessionsByDomain = useCallback(
    async (domain: string, skip: number = 0, limit: number = 10) => {
      ...
      try {
        const fetchedSessions = await auditSessionService.getSessionsByDomain(
          domain,
          skip,
          limit
        );
  ...
  const searchSessions = useCallback(
    async (searchData: AuditSessionSearchRequest) => {
      ...
      try {
        const fetchedSessions = await auditSessionService.searchSessions(
          searchData
        );
        setSessions(fetchedSessions);
      } catch (error: any) {
  ...
  const fetchSessionDocumentsSilent = useCallback(async (sessionId: string) => {
  ...
  const addDocumentToSession = useCallback(
    async (sessionId: string, documentId: string, notes?: string) => {
      ...
      try {
        ...
        const updatedDocuments = await auditSessionService.getSessionDocuments(
          sessionId
        );
  ...
  const getAuditSessionHistory = useCallback(async (sessionId: string) => {
    setLoading(true);
    clearError();

    try {
      const history = await auditSessionService.getAuditSessionHistory(
        sessionId
      );
      return history;
    } catch (error: any) {
  ...
  const closeSession = useCallback(
    async (sessionId: string, sessionSummary?: string) => {
      ...
      try {
        const updatedSession = await auditSessionService.closeAuditSession(
          sessionId,
          sessionSummary
        );
  ...
  const reactivateSession = useCallback(
    async (sessionId: string) => {
      ...
      try {
        const updatedSession = await auditSessionService.activateAuditSession(
          sessionId
        );
        setCurrentSession(updatedSession);
        // Update in sessions list if present
